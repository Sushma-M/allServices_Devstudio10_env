...
public class JobCandidateServiceImpl implements JobCandidateService {
    ...
    @Transactional(value = "AdventureWorks2014TransactionManager")
    @Override
    public JobCandidate partialUpdate(Integer jobcandidateId, Map<String, Object>jobCandidatePatch) {
        LOGGER.debug("Partially Updating the JobCandidate with id: {}", jobcandidateId);
        
        JobCandidate jobCandidate = getById(jobcandidateId);

        try {
            ObjectReader jobCandidateReader = this.objectMapper.readerForUpdating(jobCandidate);
            jobCandidate = jobCandidateReader.readValue(this.objectMapper.writeValueAsString(jobCandidatePatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", jobCandidatePatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        jobCandidate = update(jobCandidate);
    ...
    public JobCandidate delete(Integer jobcandidateId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), JobCandidate.class.getSimpleName(), jobcandidateId);
        }
