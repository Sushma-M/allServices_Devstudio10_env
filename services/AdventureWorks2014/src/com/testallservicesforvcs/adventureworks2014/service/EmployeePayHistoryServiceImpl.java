...
public class EmployeePayHistoryServiceImpl implements EmployeePayHistoryService {
    ...
    @Transactional(value = "AdventureWorks2014TransactionManager")
    @Override
    public EmployeePayHistory partialUpdate(EmployeePayHistoryId employeepayhistoryId, Map<String, Object>employeePayHistoryPatch) {
        LOGGER.debug("Partially Updating the EmployeePayHistory with id: {}", employeepayhistoryId);
        
        EmployeePayHistory employeePayHistory = getById(employeepayhistoryId);

        try {
            ObjectReader employeePayHistoryReader = this.objectMapper.readerForUpdating(employeePayHistory);
            employeePayHistory = employeePayHistoryReader.readValue(this.objectMapper.writeValueAsString(employeePayHistoryPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", employeePayHistoryPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        employeePayHistory = update(employeePayHistory);
    ...
    public EmployeePayHistory delete(EmployeePayHistoryId employeepayhistoryId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), EmployeePayHistory.class.getSimpleName(), employeepayhistoryId);
        }
