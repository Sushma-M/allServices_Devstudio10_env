...
public class VjobCandidateServiceImpl implements VjobCandidateService {
    ...
    @Transactional(value = "AdventureWorks2014TransactionManager")
    @Override
    public VjobCandidate partialUpdate(VjobCandidateId vjobcandidateId, Map<String, Object>vjobCandidatePatch) {
        LOGGER.debug("Partially Updating the VjobCandidate with id: {}", vjobcandidateId);
        
        VjobCandidate vjobCandidate = getById(vjobcandidateId);

        try {
            ObjectReader vjobCandidateReader = this.objectMapper.readerForUpdating(vjobCandidate);
            vjobCandidate = vjobCandidateReader.readValue(this.objectMapper.writeValueAsString(vjobCandidatePatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", vjobCandidatePatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        vjobCandidate = update(vjobCandidate);
    ...
    public VjobCandidate delete(VjobCandidateId vjobcandidateId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), VjobCandidate.class.getSimpleName(), vjobcandidateId);
        }
